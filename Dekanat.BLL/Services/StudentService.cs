...
namespace Dekanat.BLL.Services
{
    public class StudentService : IStudentService
    {
        ...
        public List<StudentDTO> SearchByGroup(string nam)
        {
            var mapper = new MapperConfiguration(cfg => cfg.CreateMap<Student, StudentDTO>()).CreateMapper();
            return mapper.Map<IEnumerable<Student>, List<StudentDTO>>(unitRep.studentRepository.SearchByGroup(nam));

        }

        public List<StudentDTO> SearchByName(string nam)
        {
            var mapper = new MapperConfiguration(cfg => cfg.CreateMap<Student, StudentDTO>()).CreateMapper();
            return mapper.Map<IEnumerable<Student>, List<StudentDTO>>(unitRep.studentRepository.SearchByName(nam));
