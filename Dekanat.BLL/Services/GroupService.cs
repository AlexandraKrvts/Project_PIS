...
namespace Dekanat.BLL.Services
{
    public class GroupService : IGroupService
    {
        ...
        public List<GroupDTO> SearchByFaculty(string faculty)
        {
            var mapper = new MapperConfiguration(cfg => cfg.CreateMap<Group, GroupDTO>()).CreateMapper();
            return mapper.Map<IEnumerable<Group>, List<GroupDTO>>(unitRep.groupRepository.SearchByFaculty(faculty));
        }

        public List<GroupDTO> SearchByName(string nam)
        {
            var mapper = new MapperConfiguration(cfg => cfg.CreateMap<Group, GroupDTO>()).CreateMapper();
            return mapper.Map<IEnumerable<Group>, List<GroupDTO>>(unitRep.groupRepository.SearchByName(nam));
