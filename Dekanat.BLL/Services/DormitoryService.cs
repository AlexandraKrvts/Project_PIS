...
namespace Dekanat.BLL.Services
{
    public class DormitoryService : IDormitoryService
    {
        ...
        public List<DormitoryDTO> SearchByRoomsAmount(int amount)
        {
            var config = new MapperConfiguration(cfg => {
                cfg.CreateMap<Dormitory, DormitoryDTO>();
            });
            IMapper mapper = config.CreateMapper();
            return mapper.Map<IEnumerable<Dormitory>, List<DormitoryDTO>>(unitRep.dormitoryRepository.SearchByRoomsAmount(amount));
